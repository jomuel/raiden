...
A pathfinding service is a third party helping your node with efficient transfer routing. It is usually paid in RDN tokens.

.. note::
    The Raiden Service Bundle, with pathfinding service and registry, will not be deployed on mainnet until the Alderaan
    release. Please use the otherwise discouraged ``--routing-mode local`` in the meantime.
